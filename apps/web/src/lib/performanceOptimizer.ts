...
class PerformanceOptimizer {
  ...
  async recordPerformanceMetrics(
    ...
    try {
      ...
      await monitoringSystem.recordGauge(
        'bundle_size_kb',
        fullMetrics.bundleSize
      );
      await monitoringSystem.recordGauge(
        'page_load_time_ms',
        fullMetrics.loadTime
      );
      await monitoringSystem.recordGauge(
        'first_contentful_paint_ms',
        fullMetrics.firstContentfulPaint
      );
      await monitoringSystem.recordGauge(
        'largest_contentful_paint_ms',
        fullMetrics.largestContentfulPaint
      );
      await monitoringSystem.recordGauge(
        'cumulative_layout_shift',
        fullMetrics.cumulativeLayoutShift
      );
      await monitoringSystem.recordGauge(
        'first_input_delay_ms',
        fullMetrics.firstInputDelay
      );
      await monitoringSystem.recordGauge(
        'time_to_interactive_ms',
        fullMetrics.timeToInteractive
      );
      await monitoringSystem.recordGauge(
        'memory_usage_mb',
  ...
  async generatePerformanceReport(): Promise<{
    ...
    try {
      ...
      const relevantRecommendations =
        this.getRelevantRecommendations(avgMetrics);
    ...
    const sums = metrics.reduce(
      (acc, m) => ({
        ...
        largestContentfulPaint:
          acc.largestContentfulPaint + m.largestContentfulPaint,
        cumulativeLayoutShift:
          acc.cumulativeLayoutShift + m.cumulativeLayoutShift,
  ...
  private getRelevantRecommendations(
    metrics: PerformanceMetrics
  ): OptimizationRecommendation[] {
