...
export class ModelAdvisor {
  ...
  async getOptimalModel(
    ...
    try {
      ...
      if (error) {
        throw new Error(
          `Failed to get model recommendations: ${error.message}`
        );
      }
      ...
      const affordableModels = data.filter(
        (model: any) => model.total_cost <= maxCostUsd
      );
      ...
      const selectedModel = this.selectModelForUseCase(
        affordableModels,
        useCase
      );
  ...
  async getModelPerformance(
    ...
    if (
      ...
      this.performanceCache.get(cacheKey).expiry > now
    ) {
    ...
    try {
      ...
      const performance: ModelPerformance[] = Array.from(
        modelStats.values()
      ).map(stats => {
        ...
        const successRate =
          stats.total_requests > 0
            ...
            : 0;
        const qualityRating =
          stats.quality_ratings.length > 0
            ? stats.quality_ratings.reduce(
                (sum: number, rating: number) => sum + rating,
                0
              ) / stats.quality_ratings.length
  ...
  async updateModelPerformance(
    ...
    try {
      ...
      if (this.performanceCache.has(cacheKey)) {
        ...
        const modelIndex = cached.data.findIndex(
        ...
        if (modelIndex >= 0) {
          ...
          if (qualityRating) {
            ...
            model.quality_rating =
              model.quality_ratings.reduce(
                (sum: number, rating: number) => sum + rating,
                0
              ) / model.quality_ratings.length;
          }

          model.average_latency = Math.round(
            model.total_latency / model.total_requests
          );
          model.average_cost =
            Math.round((model.total_cost / model.total_requests) * 10000) /
            10000;
          model.success_rate =
            Math.round(
              (model.successful_requests / model.total_requests) * 100
            ) / 100;
      ...
      await supabase.from('ai_evolution_logs').insert({
        tenant_id: 'system',
  ...
  async getCostComparison(
    ...
    Array<{ model: string; cost: number; quality: number; latency: number }>
  > {
    try {
  ...
  private selectModelForUseCase(models: any[], useCase: string): any {
    switch (useCase) {
      ...
      case 'quality_optimized':
        return models.reduce((max, model) =>
          model.quality_score > max.quality_score ? model : max
        );

      case 'speed_optimized':
        return models.reduce((min, model) =>
          model.latency_ms < min.latency_ms ? model : min
        );
      ...
      case 'balanced':
        ...
        return models.reduce((best, model) => {
          const bestScore =
            ...
            best.total_cost * 0.3 -
            (best.latency_ms / 10000) * 0.3;
          const modelScore =
            model.quality_score * 0.4 -
            model.total_cost * 0.3 -
            (model.latency_ms / 10000) * 0.3;
  ...
  private generateReasoning(
    ...
    reasoning.push(
      `Cost: $${model.total_cost.toFixed(4)} for ${estimatedTokens} tokens`
    );
