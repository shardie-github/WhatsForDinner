#!/usr/bin/env node
/**
 * ETL Script: Pull Ads Data from TikTok
 * 
 * Fetches ad spend, impressions, clicks, conversions from TikTok Marketing API
 * and inserts into Supabase spend table.
 * 
 * Usage:
 *   node scripts/etl/pull_ads_tiktok.ts [--dry-run] [--start-date YYYY-MM-DD] [--end-date YYYY-MM-DD]
 * 
 * Environment Variables:
 *   TIKTOK_TOKEN - TikTok Marketing API access token
...
/**
 * Fetch ad data from TikTok Marketing API
 */
async function fetchTikTokAds(
  ...
  // Placeholder: Replace with actual TikTok Marketing API call
  // Example API endpoint: https://business-api.tiktok.com/open_api/v1.3/report/integrated/get/
  const url = 'https://business-api.tiktok.com/open_api/v1.3/report/integrated/get/';
  
  console.log(`Fetching TikTok ads data from ${startDate} to ${endDate}...`);
  ...
  return withRetry(async () => {
    ...
    return (data.data?.list || []).map((item: any) => ({
      ...
      spend: parseFloat(item.metrics?.spend || '0') * 100, // Convert to cents
      ...
      conversion_value: parseFloat(item.metrics?.conversion_value || '0') * 100,
      date: item.dimensions?.stat_time_day || item.dimensions?.stat_time_hour?.split(' ')[0],
      hour: item.dimensions?.stat_time_hour ? parseInt(item.dimensions.stat_time_hour.split(' ')[1]) : undefined,
    }));
  });
}
  ...
  for (const ad of adData) {
    try {
      const spendRecord = {
        ...
        ad_set_id: ad.adgroup_id,
        ad_set_name: ad.adgroup_name,
        ad_id: ad.ad_id,
        ad_name: ad.ad_name,
...
async function main() {
  ...
  console.log('TikTok Ads ETL Job');
  console.log('='.repeat(60));
  ...
  try {
    ...
    if (!tiktokToken) {
      throw new Error('TIKTOK_TOKEN environment variable is required');
    }
    if (!advertiserId) {
    ...
    // Fetch ad data from TikTok
    const adData = await fetchTikTokAds(tiktokToken, advertiserId, startDate, endDate);
    console.log(`Fetched ${adData.length} ad records from TikTok`);
...
export { fetchTikTokAds, insertSpendData, withRetry };
