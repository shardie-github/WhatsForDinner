...
const logger = createComponentLogger('cadence-orchestrator-ts');
...
(async()=>{
  ...
  if(!pass){ const t=new Date().toISOString().replace(/[:T]/g,"-").slice(0,19); const ticket=`${BACKLOG_DIR}/READY_cadence_fix_${t}.md`;
    wfile(ticket,`# Cadence Fix Ticket\nIssue: cadence tasks failed\nWhen: ${nowIso()}\nOwner: Platform/Data\nKPI: All green on Verify + DQ + Schedules\nSee: ${report}\n`); logger.error('❌ Cadence failed. Ticket:', { ticket }); process.exit(1);
  } else { logger.info('✅ Cadence orchestration passed.'); }
