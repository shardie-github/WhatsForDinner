...
import { dsarRequests } from '../db/schema.js';
...
// HELPERS
// ============================================================================
function computeDeadline(region) {
...
async function sendVerificationEmail(email, requestId) {
    const token = jwt.sign({
        email,
        request_id: requestId,
        type: 'dsar_verification',
    }, DSAR_VERIFICATION_JWT_SECRET, { expiresIn: '24h' });
...
export async function POST(request) {
    try {
        ...
        if (existing.length > 0) {
            return NextResponse.json({ error: 'You already have a pending request. Please verify it first.' }, { status: 409 });
        }
 ...
 */
export async function verifyDSAR(request) {
    try {
        const body = await request.json();
        const { token } = verifyDSARSchema.parse(body);
        let decoded;
        try {
            decoded = jwt.verify(token, DSAR_VERIFICATION_JWT_SECRET);
        }
        catch {
        ...
        if (dsarRequest.status !== 'received') {
            return NextResponse.json({ error: `Request already ${dsarRequest.status}` }, { status: 400 });
        }
 ...
 */
export async function getMyDSARs(request) {
    try {
        const authHeader = request.headers.get('authorization');
        if (!authHeader?.startsWith('Bearer ')) {
 ...
 */
export async function fulfillDSAR(request) {
    try {
 ...
 */
export async function eraseDSAR(request) {
    try {
        const auth = await getAdminAuth(request);
