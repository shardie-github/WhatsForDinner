...
import { referralPrograms, referralCodes, referrals, users, promoOffers, lifecycleEvents, } from '../db/schema.js';
...
function generateReferralCode(userId, programId) {
 ...
 */
export async function POST_PROGRAMS(request) {
    try {
        ...
        const isAdmin = user[0]?.preferences && user[0].preferences.role === 'admin';
        if (!isAdmin) {
        ...
        if ((error instanceof z.ZodError) || (error instanceof Error)) {
            return NextResponse.json({ error: 'Invalid input', details: error instanceof z.ZodError ? error.errors : error.message }, { status: 400 });
        }
 ...
 */
export async function POST_CODE(request) {
    try {
        const ctx = await getAuthContext(request);
        if (!ctx?.user) {
            return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
        ...
        const [existingCode] = await db
            .select()
            .from(referralCodes)
            .where(and(eq(referralCodes.program_id, program.id), eq(referralCodes.owner_user_id, ctx.user.id)))
            .limit(1);
        if (existingCode) {
...
export async function POST_TRACK(request) {
    try {
        ...
        const [codeRow] = await db
            ...
            .where(and(eq(referralCodes.code, validated.code), eq(referralPrograms.active, true)))
            .limit(1);
        if (!codeRow) {
        ...
        const existingWhere = validated.referee_email
            ? and(eq(referrals.code_id, codeRow.code_id), eq(referrals.referee_email, validated.referee_email))
            : and(eq(referrals.code_id, codeRow.code_id), isNotNull(referrals.referee_email), sql `${referrals.referee_email} = ''`);
 ...
 */
export async function POST_CLAIM(request) {
    try {
        const ctx = await getAuthContext(request);
        const body = await request.json();
        ...
        const existing = await db
            .select()
            .from(referrals)
            .where(and(eq(referrals.code_id, codeRow.code_id), or(refereeUserId ? eq(referrals.referee_user_id, refereeUserId) : sql `false`, refereeEmail ? eq(referrals.referee_email, refereeEmail) : sql `false`), sql `${referrals.status} IN ('signed_up', 'converted')`))
            .limit(1);
        ...
        if (program[0]?.reward_receiver && Object.keys(program[0].reward_receiver).length > 0) {
            const reward = program[0].reward_receiver;
