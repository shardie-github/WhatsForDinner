...
import { vanWestendorpSurveys, priceExperiments, elasticityResults, } from '../db/schema.js';
 ...
 */
export async function GET_CURRENT(request) {
    try {
        ...
        const recommendation = await getRecommendedPrice(params.plan, params.country, params.platform, params.currency);
 ...
 */
export async function POST_SURVEY(request) {
    try {
        const ctx = await getAuthContext(request);
        const body = await request.json();
        const data = surveySchema.parse(body);
        // Validate: too_cheap < cheap < expensive < too_expensive
        if (!(data.too_cheap < data.cheap &&
            data.cheap < data.expensive &&
            data.expensive < data.too_expensive)) {
            return NextResponse.json({ error: 'Invalid price points: must satisfy too_cheap < cheap < expensive < too_expensive' }, { status: 400 });
        }
 ...
 */
export async function GET_REVENUE_SUMMARY(request) {
    try {
        const ctx = await getAuthContext(request);
        if (!ctx?.user) {
 ...
 */
export async function GET_ELASTICITY(request, params) {
    try {
        const ctx = await getAuthContext(request);
        if (!ctx?.user) {
            return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
        ...
        const [result] = await db
            ...
            .where(and(or(isNull(elasticityResults.country), eq(elasticityResults.country, params.country)), eq(elasticityResults.plan, params.plan)))
 ...
 */
export async function POST_PRICE_EXPERIMENT(request) {
    try {
        const ctx = await getAuthContext(request);
        if (!ctx?.user) {
            return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
