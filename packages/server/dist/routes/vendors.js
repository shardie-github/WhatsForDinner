...
export async function GET(request) {
    try {
        ...
        const filters = [];
        if (category) {
            filters.push(eq(vendorCatalog.category, category));
        }
        if (status) {
            filters.push(eq(vendorCatalog.status, status));
        }
        if (riskLevel) {
            filters.push(eq(vendorCatalog.risk_level, riskLevel));
        }
...
export async function PUT(request) {
    try {
        ...
        if (data.dpa_url)
            updateData.dpa_url = data.dpa_url;
        if (data.subprocessor !== undefined)
            updateData.subprocessor = data.subprocessor;
        if (data.pii_access !== undefined)
            updateData.pii_access = data.pii_access;
        if (data.risk_level)
 ...
 */
async function disableVendorFeatures(vendorName, category) {
 ...
 * Get vendor details
 */
export async function getVendor(request) {
    try {
        const auth = await getAdminAuth(request);
        if (!auth || !requirePermission(auth.admin.role, 'vendor:read')) {
            return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
