...
export const builtInJourneys = [
    {
        ...
        steps: [
            ...
            },
            {
                ...
                trigger: async (user, state) => {
                    if (!state || state.step !== 'planner_tips')
                        return false;
        ...
        steps: [
            ...
            },
            {
                ...
                trigger: async (user, state) => {
                    if (user.plan === 'premium')
                        return false;
                    if (!state || state.step !== 'contextual_upsell')
                        return false;
        ...
        steps: [
            {
                ...
                trigger: async (user) => {
                    ...
                    const recentEvents = await db
                        ...
                        .where(and(eq(lifecycleEvents.user_id, user.id), eq(lifecycleEvents.name, 'PurchaseStarted'), // User started purchase but didn't complete
 ...
 */
async function getJourneyState(userId, journeyKey) {
 ...
 */
async function updateJourneyState(userId, journeyKey, step, meta) {
    await db
 ...
 */
async function sendEmail(user, eventName, options) {
    try {
 ...
 */
export async function processJourney(userId, journey) {
    try {
 ...
 */
export async function triggerJourneyStep(userId, journeyKey, stepKey) {
