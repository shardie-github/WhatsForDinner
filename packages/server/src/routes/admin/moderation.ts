 ...
 */
export async function GET_MODERATION_QUEUE(request: NextRequest) {
  try {
    ...
    if (status) {
      conditions.push(eq(moderationQueue.status, status as any));
    }
    if (entityKind) {
      conditions.push(eq(moderationQueue.entity_kind, entityKind as any));
    }
    if (priority) {
      conditions.push(eq(moderationQueue.priority, priority as any));
    }
    if (assignedTo) {
 ...
 */
export async function POST_MODERATION_ASSIGN(request: NextRequest) {
  try {
    const adminAuth = await getAdminAuth(request);
    if (!adminAuth) {
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
 ...
 */
export async function PATCH_MODERATION_RESOLVE(request: NextRequest) {
  try {
    const adminAuth = await getAdminAuth(request);
    if (!adminAuth) {
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
 ...
 */
export async function POST_MODERATION_CREATE(request: NextRequest) {
  try {
    const adminAuth = await getAdminAuth(request);
    if (!adminAuth) {
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    ...
    const [existing] = await db
      ...
      .where(
        and(
          eq(moderationQueue.entity_kind, data.entity_kind),
          eq(moderationQueue.entity_id, data.entity_id),
          eq(moderationQueue.status, 'open'),
        ),
      )
      .limit(1);
