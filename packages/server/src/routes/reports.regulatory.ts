...
export async function generateRegulatoryReport(
...
): Promise<{ reportId: string; url: string; checksum: string }> {
  // Collect metrics
  const dsarCounts = await db
    ...
    .where(
      and(
        eq(dsarRequests.region, region),
        gte(dsarRequests.submitted_at, periodStart),
        lte(dsarRequests.submitted_at, periodEnd),
      ),
    )
  ...
  const completedWithinDeadline = await db
    ...
    .where(
      and(
        ...
        gte(dsarRequests.submitted_at, periodStart),
        lte(dsarRequests.submitted_at, periodEnd),
        sql`${dsarRequests.completed_at} <= ${dsarRequests.window_deadline}`,
      ),
    );
  ...
  const vendorChanges = await db
    ...
    .where(
      and(
        gte(vendorCatalog.updated_at, periodStart),
        lte(vendorCatalog.updated_at, periodEnd),
      ),
    );
  ...
  const riskUpdates = await db
    ...
    .where(
      and(
        gte(riskRegister.updated_at, periodStart),
        lte(riskRegister.updated_at, periodEnd),
      ),
    );
  ...
  try {
    ...
    if (!period_start || !period_end || !region) {
      return NextResponse.json(
        { error: 'period_start, period_end, and region required' },
        { status: 400 },
      );
    }
 ...
 */
export async function downloadReport(request: NextRequest) {
  try {
    const auth = await getAdminAuth(request);
    if (!auth || !requirePermission(auth.admin.role, 'reports:read')) {
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
