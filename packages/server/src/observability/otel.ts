 ...
 */
export async function initOpenTelemetrySDK(config: OTELConfig = {}): Promise<void> {
  if (sdk) {
 ...
 * Get global tracer
 */
export function getTracer(name: string = 'nomad-backend') {
 ...
 * Get global meter
 */
export function getMeter(name: string = 'nomad-backend') {
...
export async function withSpan<T>(
  name: string,
  fn: (span: ReturnType<typeof getTracer> extends infer T ? T : never) => Promise<T>,
): Promise<T> {
