...
export async function runPayoutCycle(
  ...
  periodEnd: Date,
  partnerId?: string,
): Promise<{ processed: number; failed: number; totalPayoutCents: number }> {
  let processed = 0;
  let failed = 0;
  ...
  for (const partner of activePartners) {
    try {
      ...
      const [existing] = await db
        ...
        .where(
          and(
            eq(payouts.partner_id, partner.id),
            eq(payouts.period_start, periodStart.toISOString().split('T')[0]),
            eq(payouts.period_end, periodEnd.toISOString().split('T')[0])
          )
        )
        .limit(1);
 ...
 */
async function computePartnerPayout(
  partnerId: string,
  periodStart: Date,
  periodEnd: Date,
): Promise<{
  ...
  const periodConversions = await db
    ...
    .where(
      and(
        ...
        gte(conversions.ts, periodStart),
        lte(conversions.ts, periodEnd)
      )
    );
 ...
 */
export async function runPayoutCycleHandler(periodStart?: string, periodEnd?: string, partnerId?: string) {
