...
export interface ControlDefinition {
  ...
  testMethod: () => Promise<{ pass: boolean; artifact?: string; meta?: Record<string, unknown> }>;
}

/**
 * Registry of controls
 */
const CONTROL_REGISTRY: ControlDefinition[] = [
  // Access Control
  {
    key: 'AC-1',
    framework: 'soc2',
 ...
 */
export async function runCollector(control: ControlDefinition): Promise<void> {
 ...
 */
export async function bootstrapControls(): Promise<void> {
 ...
 */
export async function runControlsCheck(frequency?: 'continuous' | 'daily' | 'weekly' | 'monthly' | 'quarterly'): Promise<{
  checked: number;
  passed: number;
  failed: number;
  errors: number;
  ...
  for (const controlRecord of controlsToCheck) {
    ...
    if (controlRecord.last_checked_at) {
      const hoursSinceCheck =
        (Date.now() - new Date(controlRecord.last_checked_at).getTime()) / (1000 * 60 * 60);

      let minHours = 24;
      if (controlDef.frequency === 'continuous') minHours = 1;
      else if (controlDef.frequency === 'daily') minHours = 24;
      else if (controlDef.frequency === 'weekly') minHours = 168;
      else if (controlDef.frequency === 'monthly') minHours = 720;
      else if (controlDef.frequency === 'quarterly') minHours = 2160;
 ...
 */
async function sendAlert(control: typeof controls.$inferSelect, message: string): Promise<void> {
