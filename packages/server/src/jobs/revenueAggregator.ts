 ...
 * Runs at 00:05 UTC
 */
export async function dailyRevenueAggregation(): Promise<void> {
 ...
 */
export async function weeklyLTVSegmentation(): Promise<void> {
  ...
  try {
    ...
    const newUsers = await db
      .select({
        ...
        avgCAC: sql<number>`5000`, // Default CAC estimate
        count: count(),
      })
      .from(transactions)
      .where(
        and(
          ...
          gte(transactions.ts, twelveWeeksAgo),
          sql`${transactions.ts} = (
    ...
    const retainedUsers = await db
      .select({
        ...
        `,
        avgCAC: sql<number>`5000`,
      })
      .from(transactions)
      .where(
        and(
          eq(transactions.status, 'success'),
          gte(transactions.ts, fourWeeksAgo),
          sql`EXISTS (
    ...
    const churnedUsers = await db
      ...
      .where(
        and(
          eq(transactions.status, 'success'),
          lte(transactions.ts, eightWeeksAgo),
          gte(transactions.ts, twelveWeeksAgo),
          sql`NOT EXISTS (
            SELECT 1 FROM ${transactions} t2
            WHERE t2.user_id = ${transactions.user_id}
    ...
    const reactivatedUsers = await db
      .select({
        ...
        `,
        avgCAC: sql<number>`3000`, // Lower CAC for reactivated
      })
      .from(transactions)
      .where(
        and(
