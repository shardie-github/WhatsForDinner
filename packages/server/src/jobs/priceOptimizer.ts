 ...
 */
async function shouldPauseExperiment(experimentSlug: string): Promise<boolean> {
  const [experiment] = await db
    .select()
    .from(priceExperiments)
    .where(
      and(
        eq(priceExperiments.slug, experimentSlug),
        eq(priceExperiments.status, 'running'),
      ),
    )
    .limit(1);

  if (!experiment || !experiment.started_at) return false;
  ...
  if (startedAt > oneWeekAgo) return false;
  ...
  const baselineConversions = await db
    ...
    .where(
      and(
        eq(lifecycleEvents.name, 'PaywallCTA'),
        gte(lifecycleEvents.ts, baselineStart),
        lte(lifecycleEvents.ts, baselineEnd),
      ),
    )
    .limit(1);
  ...
  const baselineRevenue = await db
    ...
    .where(
      and(
        ...
        gte(transactions.ts, baselineStart),
        lte(transactions.ts, baselineEnd),
      ),
    )
    .limit(1);
  ...
  const experimentConversions = await db
    ...
    .where(
      and(
        eq(lifecycleEvents.name, 'PaywallCTA'),
        gte(lifecycleEvents.ts, oneWeekAgo),
      ),
    )
    .limit(1);
  ...
  const experimentRevenue = await db
    ...
    .where(
      and(
        eq(transactions.status, 'success'),
        gte(transactions.ts, oneWeekAgo),
      ),
    )
    .limit(1);
 ...
 */
export async function optimizePrices(): Promise<void> {
    ...
    for (const data of elasticityData) {
      ...
      if (existingExperiment.length > 0) {
        ...
        if (shouldPause) {
          ...
          logger.info(
            { slug: existingExperiment[0].slug },
            'Auto-paused price experiment due to performance drop',
          );
        }
        continue;
      }
      ...
      logger.info(
        { slug, country, plan, variantA, variantB, elasticity },
        'Created automated price experiment',
      );
    }
