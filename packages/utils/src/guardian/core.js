...
export class Guardian {
    ...
    ledgerPath;
    policyPath;
    constructor(userId, ledgerDir = './guardian/logs') {
    ...
    loadPolicies() {
        try {
            ...
            if (process.env.NODE_ENV === 'development') {
                logger.warn('Failed to load policies', { message: 'using defaults', error });
            }
            return this.getDefaultPolicies();
        }
    }
    normalizeDataClasses(classes) {
        const normalized = {};
        for (const [key, value] of Object.entries(classes || {})) {
            normalized[key] = {
                riskWeight: value.risk_weight || 0.5,
                defaultAction: value.default_action || 'allow',
                requiresConsent: value.requires_consent || false,
            };
        }
        return normalized;
    }
     ...
     */
    assessRisk(scope, dataClass, metadata = {}) {
        ...
        if (metadata.payloadSize) {
            const size = metadata.payloadSize;
        ...
        const riskScore = Math.min(100, baseRisk * scopeMultiplier * this.policies.riskWeights.impact * impact +
            baseRisk * scopeMultiplier * this.policies.riskWeights.likelihood * likelihood);
        const riskLevel = riskScore >= 85 ? 'high' : riskScore >= 50 ? 'medium' : 'low';
        const guardianAction = this.determineAction(riskScore);
        const explanation = this.generateExplanation(scope, dataClass, riskLevel, guardianAction, metadata);
        return {
    ...
    determineAction(riskScore) {
        const actions = ['allow', 'mask', 'redact', 'block', 'alert'];
    ...
    generateExplanation(scope, dataClass, riskLevel, action, metadata) {
        const classNames = {
     ...
     */
    async processEvent(userId, scope, dataClass, action, target, metadata = {}) {
    ...
    getLastLedgerHash() {
        try {
            ...
            if (process.env.NODE_ENV === 'development') {
                logger.warn('Failed to read last ledger hash:', { error });
            }
            return undefined;
        }
    }
    async appendToLedger(event) {
        const ledgerEntry = {
