...
try {
  ...
  const trufflehogOutput = execSync('trufflehog filesystem . --no-verification --format json', { encoding: 'utf8' })
...
try {
  ...
  auditReport.checks.environmentFiles = {
    ...
    details: {
      ...
      envExists,
      envLocalExists,
      recommendation: 'Use .env.example for documentation, avoid committing .env files'
    }
  }
} catch (error) {
  auditReport.checks.environmentFiles = {
...
auditReport.status = securityScore >= 80 ? 'SECURE' : securityScore >= 60 ? 'NEEDS_ATTENTION' : 'VULNERABLE'
