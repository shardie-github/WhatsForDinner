...
async function isRLSEnabled(tableName) {
  try {
    ...
    try {
      ...
      logVerbose(`Could not check RLS for ${tableName}: ${fallbackError.message}`);
      return null;
    }
  }
}
...
function validatePolicySyntax(policy) {
  ...
  if (policy.qual && !policy.qual.includes('auth.uid()') && !policy.qual.includes('current_user')) {
...
function generateReport(auditResults) {
  const report = {
    ...
    summary: {
      ...
      totalPolicies: auditResults.reduce((sum, r) => sum + r.policies.length, 0),
...
async function auditRLS() {
  ...
  for (const table of tables) {
    ...
    for (const policy of policies) {
      ...
      issues.push(...policyIssues.map(issue => `${policy.policyname}: ${issue}`));
    }
    ...
    logVerbose(`Table ${table.table_name}: RLS=${rlsEnabled}, Policies=${policies.length}, Issues=${issues.length}`);
  }
