...
const CommerceHubRequestSchema = z.object({
  action: z.enum(['summary', 'reconcile', 'pricing_suggestion', 'revenue_analysis']),
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { action, period, region, tenant_id, partner_id } = CommerceHubRequestSchema.parse(body)
    ...
    switch (action) {
      case 'summary':
        result = await commerceHub.getFinancialSummary(requestTenantId, period, region)
        break
      case 'reconcile':
        result = await commerceHub.reconcileInvoices(requestTenantId, period)
        break
      case 'pricing_suggestion':
        result = await commerceHub.getPricingSuggestions(requestTenantId, region)
        break
      case 'revenue_analysis':
        result = await commerceHub.analyzeRevenueStreams(requestTenantId, period, partner_id)
        break
      default:
