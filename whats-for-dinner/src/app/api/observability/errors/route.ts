...
export async function GET(request: NextRequest) {
  try {
    ...
    const groupedByName = errors.reduce((acc, error) => {
      if (!acc[error.name]) {
        acc[error.name] = []
      }
    ...
    const stats = {
      total: errors.length,
      bySeverity: Object.entries(groupedErrors).reduce((acc, [severity, errors]) => {
        acc[severity] = errors.length
        return acc
      }, {} as Record<string, number>),
      byName: Object.entries(groupedByName).reduce((acc, [name, errors]) => {
