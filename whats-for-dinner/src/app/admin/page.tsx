...
export default function AdminDashboard() {
  ...
  const { data: stats, isLoading } = useQuery({
    ...
    queryFn: async (): Promise<DashboardStats> => {
      ...
      const totalMealsGenerated = usageData?.filter(u => u.action === 'meal_generation').length || 0
      const totalTokensUsed = usageData?.reduce((sum, u) => sum + (u.tokens_used || 0), 0) || 0
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Meals Generated</CardTitle>
            <Activity className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{stats.totalMealsGenerated}</div>
            <p className="text-xs text-muted-foreground">
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-bold">{stats.activeSubscriptions}</span>
              </div>
              <div className="flex items-center justify-between">
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-bold">{stats.dailyActiveUsers}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">Total Tokens Used</span>
                <span className="text-sm font-bold">{stats.totalTokensUsed.toLocaleString()}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">Cost per Token</span>
                <span className="text-sm font-bold">
                  ${stats.totalTokensUsed > 0 ? (stats.totalCost / stats.totalTokensUsed * 1000).toFixed(4) : '0.0000'}/1K
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">Avg Cost per Meal</span>
                <span className="text-sm font-bold">
                  ${stats.totalMealsGenerated > 0 ? (stats.totalCost / stats.totalMealsGenerated).toFixed(4) : '0.0000'}
                </span>
              </div>
            </div>
          </CardContent>
