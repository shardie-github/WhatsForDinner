...
export default function AdminAnalyticsPage() {
  ...
  const { data: totalStats } = useQuery({
    ...
    queryFn: async () => {
      ...
      const totalMeals = usage?.filter(u => u.action === 'meal_generation').length || 0
    ...
    <div className="space-y-6">
          ...
          <CardContent>
            <div className="space-y-3">
              {cuisinePreferences?.slice(0, 10).map((cuisine, index) => (
                <div key={cuisine.cuisine_type} className="flex items-center justify-between">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Daily activity and cost trends
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            {usageData?.slice(-7).map((day) => (
              <div key={day.date} className="flex items-center justify-between p-3 border rounded-lg">
                ...
                <div className="flex items-center space-x-6">
                  ...
                  <div className="text-center">
                    <p className="text-sm font-bold">{day.tokens_used.toLocaleString()}</p>
                    <p className="text-xs text-gray-500">Tokens</p>
                  </div>
                  <div className="text-center">
                    <p className="text-sm font-bold">${day.cost_usd.toFixed(4)}</p>
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="text-center">
                <p className="text-2xl font-bold">${totalStats.avgCostPerMeal.toFixed(4)}</p>
                <p className="text-sm text-gray-500">Avg Cost per Meal</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-bold">{totalStats.avgTokensPerMeal.toFixed(0)}</p>
                <p className="text-sm text-gray-500">Avg Tokens per Meal</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-bold">
                  ${totalStats.totalTokens > 0 ? (totalStats.totalCost / totalStats.totalTokens * 1000).toFixed(4) : '0.0000'}
                </p>
