...
class FeedbackSystem {
  async submitFeedback(
    ...
    try {
      ...
      const { data, error } = await supabase.rpc('update_recipe_feedback', {
      ...
      if (error) {
        logger.error(
          ...
          { error, recipeId, userId, feedbackType },
          'api',
          'feedback'
        );
        return false;
  ...
  async generateTrainingData(
    ...
    try {
      ...
      if (metricsError) {
        logger.error(
          'Failed to fetch recipe metrics for training data',
          { error: metricsError },
          'api',
          'feedback'
        );
      ...
      if (feedbackError) {
        logger.error(
          'Failed to fetch feedback data for training',
          { error: feedbackError },
          'api',
          'feedback'
        );
      ...
      for (const metric of recipeMetrics || []) {
        const feedback = feedbackData?.find(
          f => f.recipe_id === metric.recipe_id
        );

        if (feedback && metric.recipes) {
          // Reconstruct the original prompt
          const prompt = this.reconstructPrompt(
            metric.ingredients_used,
            metric.cuisine_type
          );
          ...
          trainingData.push({
            ...
            feedback_score:
              feedback.score ||
              (feedback.feedback_type === 'thumbs_up' ? 5 : 1),
      ...
      logger.error(
        'Error generating training data',
        { error },
        'api',
        'feedback'
      );
  ...
  async getFeedbackAnalytics(timeframe: 'day' | 'week' | 'month' = 'week') {
    try {
      ...
      for (const feedback of data || []) {
        if (feedback.feedback_type === 'thumbs_up') {
          ...
          analytics.rating_distribution[
            feedback.score as keyof typeof analytics.rating_distribution
          ]++;
        }
      ...
      logger.error(
        'Error getting feedback analytics',
        { error },
        'api',
        'feedback'
      );
  ...
  async optimizePromptsBasedOnFeedback(): Promise<boolean> {
    try {
      ...
      if (trainingData.length < 10) {
        logger.info(
          'Insufficient feedback data for prompt optimization',
      ...
      const lowRatedPrompts = trainingData.filter(
        item => item.feedback_score < 3
      );
      const highRatedPrompts = trainingData.filter(
        item => item.feedback_score >= 4
      );
      ...
      logger.error(
        'Error optimizing prompts based on feedback',
        { error },
        'api',
        'feedback'
      );
  ...
  private generateImprovedPrompts(issues: string[]): string[] {
    ...
    if (issues.includes('bland')) {
      improvements.push(
        'Focus on bold, flavorful combinations and seasoning techniques'
      );
    }

    if (issues.includes('boring')) {
      improvements.push(
        'Emphasize creative presentation and unique ingredient combinations'
      );
    }

    if (issues.includes('unclear')) {
      improvements.push(
        'Provide detailed, step-by-step instructions with specific measurements'
      );
    }

    if (issues.includes('difficult')) {
      improvements.push(
        'Prioritize simple techniques and beginner-friendly methods'
      );
    }

    if (issues.includes('time-consuming')) {
      improvements.push(
        'Focus on quick preparation methods and time-saving techniques'
      );
    }
...
export function useFeedback() {
  return {
    ...
    getFeedbackAnalytics:
      feedbackSystem.getFeedbackAnalytics.bind(feedbackSystem),
  };
}
