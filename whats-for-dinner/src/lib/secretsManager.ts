...
class SecretsManager {
  ...
  async storeSecret(key: string, value: string, environment: string = 'production'): Promise<void> {
    try {
      ...
      console.log(`Secret ${key} stored successfully for environment ${environment}`)
    } catch (error) {
  ...
  async getSecret(key: string, environment: string = 'production'): Promise<string | null> {
    try {
  ...
  async rotateSecret(key: string, environment: string = 'production'): Promise<void> {
    try {
      ...
      // Log rotation
      console.log(`Secret ${key} rotated successfully for environment ${environment}`)
  ...
  private calculateNextRotation(key: string): string {
    const policy = this.rotationPolicies.find(p => p.key === key)
    if (!policy) return new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString()
  ...
  private async logRotationEvent(key: string, environment: string, newHash: string): Promise<void> {
    try {
      await this.supabase
  ...
  async getSecretChecksums(): Promise<Record<string, string>> {
    try {
      ...
      if (error) {
        throw new Error(`Failed to retrieve secret checksums: ${error.message}`)
      }
    ...
    try {
      ...
      for (const secret of data) {
        ...
        if (policy && policy.autoRotate) {
          console.log(`Auto-rotating secret ${secret.key} for environment ${secret.environment}`)
          await this.rotateSecret(secret.key, secret.environment)
        } else {
          console.warn(`Secret ${secret.key} is due for rotation but auto-rotation is disabled`)
        }
      }
    } catch (error) {
...
export function redactSensitiveData(data: any): any {
  const sensitiveKeys = [
    ...
    'openai', 'stripe', 'supabase', 'jwt', 'encryption'
  ]

  if (typeof data === 'string') {
    return sensitiveKeys.some(key => data.toLowerCase().includes(key)) ? '[REDACTED]' : data
  }
