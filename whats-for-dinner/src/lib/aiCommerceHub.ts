...
export class AICommerceHub {
   ...
   */
  async getFinancialSummary(tenantId: string, period: string, region: string): Promise<FinancialSummary> {
    try {
      ...
      // Calculate totals
      const totalApiCosts = apiUsage?.reduce((sum, record) => sum + record.cost_usd, 0) || 0
      const totalApiRevenue = apiUsage?.reduce((sum, record) => sum + record.revenue_usd, 0) || 0
      const totalPartnerRevenue = partnerRevenue?.reduce((sum, record) => sum + record.revenue_usd, 0) || 0
      
      // Calculate subscription revenue
      const subscriptionRevenue = this.calculateSubscriptionRevenue(subscriptions || [])
  ...
  async reconcileInvoices(tenantId: string, period: string): Promise<any> {
    try {
      ...
      const stripeTransactions = await this.getStripeTransactions(tenantId, startDate, endDate)

      // Reconcile data
      const reconciliation = {
        totalApiCosts: apiUsage?.reduce((sum, record) => sum + record.cost_usd, 0) || 0,
        totalApiRevenue: apiUsage?.reduce((sum, record) => sum + record.revenue_usd, 0) || 0,
   ...
   */
  async getPricingSuggestions(tenantId: string, region: string): Promise<PricingSuggestion> {
    try {
   ...
   */
  async analyzeRevenueStreams(tenantId: string, period: string, partnerId?: string): Promise<RevenueAnalysis> {
    try {
      const periodDays = this.parsePeriod(period)
      const startDate = new Date(Date.now() - periodDays * 24 * 60 * 60 * 1000)
      const endDate = new Date()
      ...
      const subscriptionRevenue = this.calculateSubscriptionRevenue(subscriptions || [])
      const apiRevenue = apiUsage?.reduce((sum, record) => sum + record.revenue_usd, 0) || 0
      ...
      const partnerRevenue = partners?.map(partner => {
        const partnerUsage = apiUsage?.filter(record => record.partner_id === partner.id) || []
        const revenue = partnerUsage.reduce((sum, record) => sum + record.revenue_usd, 0)
        return {
          name: partner.name,
          revenue,
      ...
      const regionalBreakdown = [
        { region: 'NA', revenue: totalRevenue * 0.6, percentage: 60, growth: 0.15 },
        { region: 'EU', revenue: totalRevenue * 0.25, percentage: 25, growth: 0.08 },
        { region: 'APAC', revenue: totalRevenue * 0.15, percentage: 15, growth: 0.25 }
      ]
      ...
      return {
        totalRevenue,
        revenueStreams: [
          { name: 'Subscriptions', revenue: subscriptionRevenue, percentage: (subscriptionRevenue / totalRevenue) * 100, growth: 0.12, trend: 'up' as const },
          { name: 'API Usage', revenue: apiRevenue, percentage: (apiRevenue / totalRevenue) * 100, growth: 0.18, trend: 'up' as const },
          { name: 'Partner Revenue', revenue: partnerRevenue.reduce((sum, p) => sum + p.revenue, 0), percentage: (partnerRevenue.reduce((sum, p) => sum + p.revenue, 0) / totalRevenue) * 100, growth: 0.22, trend: 'up' as const }
        ],
   ...
   */
  private calculateCostBreakdown(apiUsage: any[]): { openai: number; anthropic: number; google: number; azure: number } {
   ...
   */
  private async calculateTrends(tenantId: string, periodDays: number): Promise<any> {
   ...
   */
  private async getStripeTransactions(tenantId: string, startDate: Date, endDate: Date): Promise<any> {
