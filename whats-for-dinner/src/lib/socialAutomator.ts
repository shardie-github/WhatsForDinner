...
Format as JSON array.`
      ...
      const response = await openaiClient.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a social media trend analyst specializing in food and cooking content. Generate data-driven insights about what\'s trending in the food space.'
          },
          {
            role: 'user',
            content: prompt
  ...
  static async generateSocialContent(
    ...
    theme: string,
    contentType: 'recipe_showcase' | 'cooking_tip' | 'ingredient_spotlight' | 'meal_prep' | 'nutrition_fact' = 'recipe_showcase'
  ): Promise<SocialPost> {
    try {
      const platformPrompts = {
        twitter: 'Create a Twitter post (under 280 characters) that is engaging and shareable. Include relevant hashtags.',
        tiktok: 'Create a TikTok caption that encourages engagement and uses trending language. Include hashtags for discoverability.',
        threads: 'Create a Threads post that starts a conversation about food and cooking. Use a conversational tone.',
        instagram: 'Create an Instagram caption that tells a story and encourages interaction. Include relevant hashtags.',
        linkedin: 'Create a LinkedIn post that provides professional value about food, nutrition, or cooking. Use a more formal tone.'
      }

      const contentPrompts = {
        recipe_showcase: 'Showcase a recipe with mouth-watering description and cooking tips.',
        cooking_tip: 'Share a useful cooking tip or technique that saves time or improves results.',
        ingredient_spotlight: 'Highlight a specific ingredient with its benefits and cooking uses.',
        meal_prep: 'Share meal prep advice or showcase a meal prep strategy.',
        nutrition_fact: 'Share an interesting nutrition fact or health benefit related to food.'
      }
...
Generate the content:`
      ...
      const { content, hashtags } = this.parseSocialContent(generatedContent, platform)
   ...
   */
  private static parseSocialContent(content: string, platform: string): { content: string; hashtags: string[] } {
  ...
  static async schedulePost(
    ...
    try {
      ...
      console.log(`Post ${postId} scheduled for ${scheduledFor} on ${platform}`)
    } catch (error) {
   ...
   */
  static async getOptimalPostingTimes(platform: SocialPost['platform']): Promise<string[]> {
    const optimalTimes = {
   ...
   */
  static async generateContentStrategy(platform: SocialPost['platform']): Promise<ContentStrategy> {
    try {
...
Format as JSON.`
      ...
      const response = await openaiClient.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a social media strategist specializing in food and cooking content. Create data-driven strategies for maximum engagement.'
          },
          {
            role: 'user',
            content: prompt
   ...
   */
  static async generateViralContentIdeas(platform: SocialPost['platform']): Promise<string[]> {
    try {
...
Make them specific, actionable, and platform-appropriate.`
      ...
      const response = await openaiClient.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a viral content strategist specializing in food and cooking content. Generate ideas that are likely to get high engagement and shares.'
          },
          {
            role: 'user',
            content: prompt
   ...
   */
  static async analyzeCompetitorContent(competitorHandle: string, platform: SocialPost['platform']): Promise<{
    top_hashtags: string[]
...
Format as JSON with arrays for each category.`
      ...
      const response = await openaiClient.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a social media analyst. Analyze competitor strategies and extract actionable insights.'
          },
          {
            role: 'user',
            content: prompt
...
Format as JSON array.`
      ...
      const response = await openaiClient.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a content calendar strategist. Create engaging, varied content plans that maximize reach and engagement.'
          },
          {
            role: 'user',
            content: prompt
