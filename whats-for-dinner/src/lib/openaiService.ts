...
export async function generateRecipes({
  ...
  const systemPrompt = aiConfig?.system_prompt || "You are a professional chef and nutritionist."

  // Generate prompt using AI config templates
  const prompt = await aiConfigManager.generatePrompt('recipe_generation', {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      await logger.info('Recipe generation successful', {
        model: model,
        attempt: attempt + 1,
      ...
      // Log the error
      await logger.error(`OpenAI API attempt ${attempt + 1} failed`, {
        error: error.message,
        model: model,
        attempt: attempt + 1,
        ingredients: ingredients
      }, 'api', 'openai', error as Error)
...
function calculateConfidenceScore(recipes: any[], ingredients: string[]): number {
  ...
  const completeRecipes = recipes.filter(r => 
    r.title && r.cookTime && r.calories && r.ingredients?.length > 0 && r.steps?.length > 0
  )
