...
export interface UGCShare {
  ...
  user_id: string
  recipe_id?: string
  content_type: 'meal_card' | 'recipe_share' | 'cooking_tip' | 'ingredient_spotlight'
  title: string
  description: string
  image_url?: string
...
export class UGCGrowth {
  ...
  static async shareToSocial(
    ...
    try {
      ...
      const shareUrls = this.generateShareUrls(shareData, platforms, customMessage)
  ...
  private static generateShareUrls(
    ...
    const description = encodeURIComponent(customMessage || shareData.description)
  ...
  static async trackEngagement(
    ...
    try {
      ...
      updatedMetrics.engagement_rate = this.calculateEngagementRate(updatedMetrics)
  ...
  static async getUserUGCAnalytics(userId: string): Promise<ShareAnalytics> {
    try {
      ...
      const totalViews = shares.reduce((sum, share) => sum + share.performance_metrics.views, 0)
      const totalEngagement = shares.reduce((sum, share) => 
        sum + share.performance_metrics.likes + share.performance_metrics.shares + share.performance_metrics.clicks, 0
      )
      ...
      const conversionRate = totalViews > 0 ? 
        shares.reduce((sum, share) => sum + share.performance_metrics.conversions, 0) / totalViews : 0

      const viralCoefficient = totalShares > 0 ? totalEngagement / totalShares : 0
  ...
  static async generateShareSuggestions(
    ...
    try {
      ...
      const suggestedHashtags = [
        ...
        ...recipeData.dietary_tags.map((tag: string) => `#${tag.toLowerCase()}`),
        '#homemade',
        '#delicious',
        '#foodie'
      ]
      ...
      const optimalPlatforms = platformPerformance.length > 0 ? 
        platformPerformance : ['twitter', 'instagram', 'facebook']
  ...
  static async createShareableMealCardData(
    ...
    try {
      ...
      const cardData = {
        ...
        description: customizations?.description || `A ${recipeData.difficulty_level} ${recipeData.cuisine_type} recipe`,
        image_url: recipeData.image_url || '/default-meal-image.jpg',
        recipe_name: recipeData.recipe_name,
        cooking_time: recipeData.cooking_time,
        difficulty_level: recipeData.difficulty_level,
        cuisine_type: recipeData.cuisine_type,
        dietary_tags: recipeData.dietary_tags || [],
        nutrition_info: customizations?.include_nutrition ? recipeData.nutrition_info : null,
        ingredients: customizations?.include_ingredients ? recipeData.ingredients : null,
