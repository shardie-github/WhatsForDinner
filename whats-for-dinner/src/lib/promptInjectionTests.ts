...
class PromptInjectionTestSuite {
  ...
  async runAllTests(): Promise<{
    ...
    for (const testCase of this.testCases) {
      try {
        const safetyResult = await aiSafetyGuardrails.validateInput(testCase.input)
        ...
        console.log(`  Expected: ${testCase.expectedRisk}, Got: ${safetyResult.riskLevel}`)
    ...
    console.log('=' .repeat(50))
    console.log(`📊 Test Results: ${passedTests}/${this.testCases.length} passed`)
    console.log(`   Critical: ${summary.critical.passed}/${summary.critical.passed + summary.critical.failed} passed`)
    console.log(`   High: ${summary.high.passed}/${summary.high.passed + summary.high.failed} passed`)
    console.log(`   Medium: ${summary.medium.passed}/${summary.medium.passed + summary.medium.failed} passed`)
    console.log(`   Low: ${summary.low.passed}/${summary.low.passed + summary.low.failed} passed`)
  ...
  async runCriticalTests(): Promise<TestResult[]> {
    const criticalTests = this.testCases.filter(tc => tc.expectedRisk === 'critical')
    const results: TestResult[] = []
...
${results.filter(r => !r.passed).map(test => 
  `### ${test.testName}
- **Input:** \`${test.input}\`
...
export async function runAutomatedRedTeamTests(): Promise<void> {
  ...
  try {
    ...
    const markdownPath = path.join(process.cwd(), 'logs', 'prompt_injection_test_report.md')
