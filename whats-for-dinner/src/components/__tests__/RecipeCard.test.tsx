...
describe('RecipeCard', () => {
  ...
  it('shows and hides instructions when toggle button is clicked', async () => {
    ...
    expect(screen.getByText('Boil water and cook pasta according to package directions')).toBeInTheDocument()
    
    await user.click(screen.getByRole('button', { name: /hide instructions/i }))
    expect(screen.queryByText('Instructions:')).not.toBeInTheDocument()
  })
  ...
  it('shows remove button when isFavorite is true', () => {
    ...
    render(<RecipeCard {...defaultProps} isFavorite={true} onRemove={mockOnRemove} />)
