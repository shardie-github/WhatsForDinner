...
serve(async (req) => {
  ...
  try {
    ...
    await supabaseClient.rpc('log_job_activity', {
      ...
      log_metadata: { job_type: job.job_type, retry_count: job.job_retry_count }
    })
...
async function processMealGenerationJob(supabaseClient: any, job: JobData): Promise<any> {
  const { pantry_items, dietary_preferences, cuisine_type, meal_type, serving_size } = job.job_payload
...
async function processEmailNotificationJob(supabaseClient: any, job: JobData): Promise<any> {
...
async function processDataCleanupJob(supabaseClient: any, job: JobData): Promise<any> {
  ...
  switch (cleanup_type) {
    case 'old_jobs':
      ...
      const { data: cleanupResult } = await supabaseClient.rpc('cleanup_old_jobs', {
...
async function processAnalyticsJob(supabaseClient: any, job: JobData): Promise<any> {
  ...
  switch (analysis_type) {
    case 'popular_ingredients':
      const { data: ingredients } = await supabaseClient.rpc('get_popular_ingredients', {
        limit_count: 10
      })
      analyticsData = { popular_ingredients: ingredients }
      break
    case 'cuisine_preferences':
      const { data: cuisines } = await supabaseClient.rpc('get_cuisine_preferences')
