...
serve(async req => {
  ...
  try {
          ...
          messages: [
            {
              role: 'system',
              content:
                'You are a helpful cooking assistant that generates detailed meal recipes based on available ingredients. Always respond with valid JSON.',
            },
            {
              role: 'user',
              content: prompt,
    ...
    await supabaseClient.from('ai_cache').insert({
    ...
    // Log recipe metrics
    await supabaseClient.from('recipe_metrics').insert({
      recipe_id: recipe?.id,
      user_id: user.id,
