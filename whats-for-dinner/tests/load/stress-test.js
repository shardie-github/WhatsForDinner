...
const stressTestData = {
  ...
  cuisineTypes: ['italian', 'mexican', 'asian', 'american', 'mediterranean', 'indian', 'thai', 'french'],
...
function getRandomData() {
  return {
    user: stressTestData.users[Math.floor(Math.random() * stressTestData.users.length)],
    mealType: stressTestData.mealTypes[Math.floor(Math.random() * stressTestData.mealTypes.length)],
    cuisineType: stressTestData.cuisineTypes[Math.floor(Math.random() * stressTestData.cuisineTypes.length)],
    dietaryRestrictions: stressTestData.dietaryRestrictions
      .sort(() => 0.5 - Math.random())
      .slice(0, Math.floor(Math.random() * 3) + 1),
    cookingTime: stressTestData.cookingTimes[Math.floor(Math.random() * stressTestData.cookingTimes.length)],
    difficulty: stressTestData.difficulties[Math.floor(Math.random() * stressTestData.difficulties.length)]
  };
}
...
function makeRequest(url, method = 'GET', payload = null, timeout = 30000) {
  ...
  try {
    ...
    timeoutRate.add(1);
    return { status: 0, timings: { duration: timeout }, body: '', error: error.message };
  }
}
...
export function testExtremeConcurrency() {
  ...
  endpoints.forEach(endpoint => {
    ...
    check(response, {
      ...
      'extreme concurrency response time acceptable': (r) => r.timings.duration < 10000,
    });
...
export function testMemoryIntensiveOperations() {
  ...
  const response = makeRequest(`${BASE_URL}/api/meals/generate`, 'POST', largePayload);
  
  check(response, {
    'memory intensive request completes': (r) => r.status > 0,
    'memory intensive response time acceptable': (r) => r.timings.duration < 15000,
...
export function testResourceExhaustion() {
  ...
  const response = makeRequest(`${BASE_URL}/api/meals/generate`, 'POST', largeRequest);
...
export function testNetworkInstability() {
  ...
  check(response, {
    ...
    'timeout handled gracefully': (r) => r.status === 0 || r.timings.duration < 2000,
  });
