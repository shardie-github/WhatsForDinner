...
function getRandomUser() {
  return concurrencyTestData.users[
    Math.floor(Math.random() * concurrencyTestData.users.length)
  ];
}
...
export function testUserRegistration() {
  ...
  check(response, {
    ...
    'user registration response time acceptable': r =>
...
export function testMealGeneration() {
  ...
  const payload = {
    meal_type:
      concurrencyTestData.mealTypes[
        Math.floor(Math.random() * concurrencyTestData.mealTypes.length)
      ],
    cuisine_type:
      concurrencyTestData.cuisineTypes[
        Math.floor(Math.random() * concurrencyTestData.cuisineTypes.length)
      ],
    dietary_restrictions: ['vegetarian'],
    preferences: {
      cooking_time: 30,
...
export function testMixedOperations() {
  ...
  operations.forEach(operation => {
    ...
    check(response, {
      ...
      'mixed operation response time acceptable': r =>
        r.timings.duration < 5000,
    });
...
export function testDataConsistency() {
  ...
  const response1 = makeAuthenticatedRequest(
    `${BASE_URL}/api/user/preferences`,
    'PUT',
    updatePayload
  );
  const response2 = makeAuthenticatedRequest(
    `${BASE_URL}/api/user/preferences`,
    'PUT',
    updatePayload
  );
  ...
  check(response1, {
    ...
    'concurrent update 1 response time acceptable': r =>
      r.timings.duration < 3000,
  });

  check(response2, {
    'concurrent update 2 completes': r => r.status > 0,
    'concurrent update 2 response time acceptable': r =>
      r.timings.duration < 3000,
  });
...
export function testSessionManagement() {
  ...
  sessionOperations.forEach(operation => {
    ...
    check(response, {
      ...
      'session operation response time acceptable': r =>
...
export function testResourceLocking() {
  ...
  const lockOperations = [
    () =>
      makeAuthenticatedRequest(`${BASE_URL}/api/meals/lock`, 'POST', {
        meal_id: 'test-meal-id',
      }),
    () =>
      makeAuthenticatedRequest(`${BASE_URL}/api/meals/unlock`, 'POST', {
        meal_id: 'test-meal-id',
      }),
  ];
...
export function teardown(data) {
  ...
  console.log(
    `Data consistency errors: ${data?.dataConsistencyErrors || 'unknown'}`
  );
  console.log(`Average response time: ${data?.avgResponseTime || 'unknown'}ms`);
  console.log(
    `Peak concurrent users: ${data?.peakConcurrentUsers || 'unknown'}`
  );
}
