...
const logger = createComponentLogger('reflection-agent-ts');
...
`;
  ...
  private async evaluatePerformance(reflection: ReflectionData): Promise<void> {
    ...
    logger.info('   Score: ${reflection.evaluation.score}/100');
    logger.info('   Efficiency: ${reflection.evaluation.efficiency}/100');
    logger.info('   Stability: ${reflection.evaluation.stability}/100');
    logger.info('   Changes: ${reflection.changes.commits} commits', { ${reflection.changes.filesChanged} files` });
  }
